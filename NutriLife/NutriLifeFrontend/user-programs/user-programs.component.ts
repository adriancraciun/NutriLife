...
export class UserProgramsComponent implements OnInit {
  ...
  filterChange(name: string): void {
    if (name == "")
      name = "any"
    this.service.getDailyPlansByUserFullyFiltered(this.loggedUserId, name, this.filter.glutenFree, this.filter.lactoseFree, this.filter.diabetesFriendly).subscribe(dailyPlans => {
  ...
  filterTypeChange(name: string, type: string): void {
    if (name == "")
      name = "any"
    this.service.getDailyPlansByUserFullyFilteredByType(this.loggedUserId, name, this.filter.glutenFree, this.filter.lactoseFree, this.filter.diabetesFriendly, type).subscribe(dailyPlans => {
      this.dailyPlans = dailyPlans
      this.changeDetection.detectChanges()
    })
  }
